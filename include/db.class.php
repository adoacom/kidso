<?php
  ...
  ***/
  ...
  class DB
  {
    ...
    function fetchBySQL($sql){
     	
     	if(!$sql) return NULL;
		
		$this->result = $this->queryUniqueObject($sql);
		
		if(is_object($this->result)) return $this->result;		    	
    }
    
    function fetchAllBySQL($sql,$returnArray = false){
    ...
    function fetchAll($where = NULL,$order = NULL,$isReturnArray = false){
...
// 		echo $sql;
		...
		if($isReturnArray || !is_bool($this->result)){
			$objects = array();
    ...
    function insert($table, $data,$debug = -1) {
		...
		return $this->execute($q,$debug);//Change by Ken at 2013/02/12 16:02
